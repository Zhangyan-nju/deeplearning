...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l2 = -torch.log(torch.exp(sim(z_k,z_k_N)/tau) / denominator2)
        total_loss += (l1 + l2)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    out_left_norm = out_left / torch.linalg.norm(out_left,dim=1,keepdim=True,ord=2)
    out_right_norm = out_right /  torch.linalg.norm(out_right,dim=1,keepdim=True,ord=2)
    pos_pairs = torch.sum(torch.dot(out_left_norm, out_right_norm),dim=1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    out_norm = out / torch.linalg.norm(out,dim=1,keepdim=True,ord=2)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = None
    exponential = torch.exp(compute_sim_matrix(out) / tau)
    ...
    denom = None
    denom = torch.sum(exponential,keepdim=True,dim=1)
    ...
    loss = None
    loss = torch.sum(numerator / denom)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
