...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    similarity = get_similarity_no_loop(text_features, image_features) # [NT, NI]
    text_idxs = torch.argmax(similarity, dim=0)
    pred_classes = [class_texts[idx] for idx in text_idxs]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        similarity = get_similarity_no_loop(text_features, self.image_features) # [1, NI]
        top_indices = similarity.topk(k=k,dim=2).tolist()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        旨在将 DINO 特征向量分类为分割类别。包括模型初始化、优化器和损失函数设置。
        ...
        self.optimer = torch.optim.Adam(self.model.parameters(), lr=0.001)
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for i in range(num_iters):
            ...
            loss.backward()
            self.optimer.step()
            pred_num += torch.sum(Y_pred == Y_train).item()
            total_num += Y_pred.shape[0]
            if (i+1) % 10 == 0:
                acc = pred_num / total_num if total_num > 0 else 0.0
                print(f"step:{i+1} loss:{loss.item():.4f} acc:{acc:.4f}")
            pred_num, total_num = 0, 0
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
