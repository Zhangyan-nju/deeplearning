...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        row = torch.Tensor(range(max_len)).unsqueeze(1)
        column = (1/10000) ** ((torch.Tensor(range(0,embed_dim,2))/embed_dim).repeat_interleave(2).unsqueeze(0))
        idx = row @ column 
        pe[0,:,0::2] = torch.sin(idx[:,0::2])
        pe[0,:,1::2] = torch.cos(idx[:,1::2])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        V = self.value(value).reshape(N, T, H, self.head_dim).permute(0, 2, 1, 3)
        Q = self.query(query).reshape(N, S, H, self.head_dim).permute(0, 2, 1, 3)
        K = self.key(key).reshape(N, T, H, self.head_dim).permute(0, 2, 3, 1)
        #  2) The function torch.matmul allows you to do a batched matrix multiply.#
        #     For example, you can do (N, H, T, E/H) by (N, H, E/H, T) to yield a  #
        #     shape (N, H, T, T). For more examples, see                           #
        #     https://pytorch.org/docs/stable/generated/torch.matmul.html          #
        attn_scores = torch.matmul(Q,K)/ (self.head_dim ** 0.5) # [N,H,S,T]
        if attn_mask != None:
          attn_mask = attn_mask.unsqueeze(0).unsqueeze(1) 
          attn_scores = attn_scores.masked_fill(attn_mask == 0, float('-inf'))
        attn_probs = torch.softmax(attn_scores,dim=-1) # [N,H,S,T]
        attn_probs = self.attn_drop(attn_probs)
        #  3) For applying attn_mask, think how the scores should be modified to   #
        #     prevent a value from influencing output. Specifically, the PyTorch   #
        #     function masked_fill may come in handy.                              #
        ############################################################################
        attn_out = torch.matmul(attn_probs, V)  # (N, H, S, head_dim)
        attn_out = attn_out.permute(0, 2, 1, 3).reshape(N, S, E)  # (N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        x_spilt = x.reshape(N,C,H//P,P,W//P,P).permute(0,2,4,1,3,5) # [N,H//P,W//P,C,P,P]
        x_flat = x_spilt.reshape(N,self.num_patches,self.patch_dim)
        out = self.proj(x_flat)
        # out = self.pos_embed(x_proj)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return out
