...
class RMSNorm(nn.Module):
    ...
    # RMSNorm 层，一种计算高效的简化版层归一化（LayerNorm）变体
...
class Block(nn.Module):
    ...
    def forward(self, x, scale_shift=None):
        ...
        # 缩放和平移用于调整输出。这是一种特征融合的变体，比简单地将特征图相加更强大。
...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        x = (1 + cfg_scale) * out_cond - cfg_scale * out_uncond
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        featureOutList = []
        for i,downsample in enumerate(self.downs):
            x = downsample[0](x, context)
            featureOutList.append(x)
            x = downsample[1](x, context)
            featureOutList.append(x)
            x = downsample[2](x)
        ...
        #    - Make sure to pass the context to each ResNet block.
        ##################################################################
        n = len(featureOutList)
        for i,upsample in enumerate(self.ups):
            x = upsample[0](x)
            x = torch.cat((x, featureOutList[n-2*i-1]),dim=1)
            x = upsample[1](x, context)
            x = torch.cat((x, featureOutList[n-2*i-2]),dim=1)
            x = upsample[2](x, context)
        ##################################################################
